...
interface Props {
  ...
  onEliminar: (id: string) => void; // Asumo que tu ID es string por tu reducer
  onEditar: (p: Participante) => void; // NUEVO: La función para ir a /editar
}

export default function ListaParticipantes({ participantes, onEliminar, onEditar }: Props) {
      ...
      {participantes.map((p) => (
        ...
        >
          <div className="flex justify-between items-start mb-4">
            ...
            {/* NUEVO: Contenedor de botones Editar/Eliminar */}
            <div className="flex gap-2">
              <button 
                onClick={() => onEditar(p)}
                className="text-indigo-500 hover:text-indigo-700 hover:bg-indigo-50 px-2 py-1 rounded transition-colors text-sm font-semibold"
              >
                Editar
              </button>
              <button 
                onClick={() => onEliminar(String(p.id))}
