...
interface Props {
  onAgregar: (participante: Participante) => Promise<void>;
}
...
export default function FormularioRegistro({ onAgregar }: Props) {
  ...
  e.preventDefault();
  const nuevoParticipante = new Participante({ ...formData, id: "", edad: Number(formData.edad) });
  await onAgregar(nuevoParticipante);
  // Solo resetea si no hubo error (el Context muestra el alert y retorna sin tirar excepción)
  setFormData({ nombre: '', email: '', edad: '', pais: 'Argentina', modalidad: '', 
